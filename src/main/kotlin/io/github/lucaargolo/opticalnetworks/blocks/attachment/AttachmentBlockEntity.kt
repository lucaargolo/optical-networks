...
open class AttachmentBlockEntity(block: Block): NetworkBlockEntity(block), GhostSlot.IBlockEntity {
    ...
    override fun fromTag(state: BlockState, tag: CompoundTag) {
        ...
        ghostInv = DefaultedList.ofSize(ghostInvSize, ItemStack.EMPTY)
    ...
    fun tryToImport(slot: Int, inventory: Inventory, sampleSize: Int) {
        ...
        if(available > 0) {
            ...
            if (!inserted && !invStack.isEmpty) {
                ...
                dummyStack.count = 1
                if(currentNetwork!!.insertStack(dummyStack).isEmpty) invStack.decrement(1)
    ...
    fun tryToExport(stack: ItemStack, inventory: Inventory, sampleSize: Int, side: Direction) {
        stack.count = 1
        if(inventory is SidedInventory) {
            inventory.getAvailableSlots(side).forEach {
                if(inventory.canInsert(it, stack, side)) {
