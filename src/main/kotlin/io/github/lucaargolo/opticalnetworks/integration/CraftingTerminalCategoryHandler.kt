...
class CraftingTerminalCategoryHandler: DefaultCategoryHandler() {
    ...
    override fun handle(context: AutoTransferHandler.Context): AutoTransferHandler.Result? {
        ...
        val missinInInvList = hasItems(input)
        val missinInNetworkList = hasItems(input, container.network)

        if (!missinInNetworkList.isEmpty()) return AutoTransferHandler.Result.createFailed("error.rei.not.enough.materials", missinInNetworkList)
        ...
        if(missinInInvList.isEmpty()) ClientSidePacketRegistry.INSTANCE.sendToServer(RoughlyEnoughItemsNetwork.MOVE_ITEMS_PACKET, buf)
